...
function SafePath([string]$Rel){$full=[IO.Path]::GetFullPath((Join-Path $Auto $Rel));$root=[IO.Path]::GetFullPath($Auto)+[IO.Path]::DirectorySeparatorChar;if(-not $full.StartsWith($root,[StringComparison]::OrdinalIgnoreCase)){throw 'path escapes AutoDirector root'};return $full}
function ReadSmall([string]$Rel,[int]$Max=160000){$p=SafePath $Rel;if(-not(Test-Path $p)){return $null};$f=Get-Item $p;if($f.Length-gt$Max){return '[OMITTED: too large]'};return Get-Content $p -Raw -Encoding UTF8}
...
try{
  ...
  switch([string]$cmd.action){
    'autodirector_probe' {$pkg=Get-Content (Join-Path $Auto 'package.json') -Raw|ConvertFrom-Json;$run=$null; if(Test-Path (Join-Path $Auto '.harness\RUNNING.json')){$run=Get-Content (Join-Path $Auto '.harness\RUNNING.json') -Raw|ConvertFrom-Json};$alive=$false;if($run -and $run.pid){$alive=$null-ne(Get-Process -Id ([int]$run.pid) -ErrorAction SilentlyContinue)};$result.version=$pkg.version;$result.head=GitHead;$result.stop_present=$false;$result.running=$run;$result.running_pid_alive=$alive;$result.harness_status=if(Test-Path(Join-Path $Auto 'reports\harness\status.json')){Get-Content(Join-Path $Auto 'reports\harness\status.json')-Raw|ConvertFrom-Json}else{$null};$result.codex_available=$null-ne(Get-Command codex.cmd -ErrorAction SilentlyContinue);$result.status='PASS';$result.retryable=$false}
    'autodirector_read_fast' {$arr=@();foreach($rel in @($cmd.paths)){$arr+=@{path=[string]$rel;content=ReadSmall([string]$rel)}};$result.files=$arr;$result.head=GitHead;$result.status='PASS';$result.retryable=$false}
    ...
    'autodirector_write_checkpoint' {$rel=[string]$cmd.path;if($rel-notin@('WEBCHAT_TODO.md','reports\harness\status.json')){throw 'checkpoint path not allowed'};$p=SafePath $rel;New-Item -ItemType Directory -Force -Path(Split-Path $p)|Out-Null;[IO.File]::WriteAllText($p,[string]$cmd.content,(New-Object Text.UTF8Encoding($false)));$result.path=$rel;$result.head=GitHead;$result.status='PASS';$result.retryable=$false}
    'autodirector_harness_start' {$run=$null;if(Test-Path(Join-Path $Auto '.harness\RUNNING.json')){$run=Get-Content(Join-Path $Auto '.harness\RUNNING.json')-Raw|ConvertFrom-Json};$alive=$false;if($run-and$run.pid){$alive=$null-ne(Get-Process -Id([int]$run.pid)-ErrorAction SilentlyContinue)};if(-not$alive){$r=RunProc 'npm.cmd' @('run','harness:start') '' 120;$result.start_exit=$r.exit;$result.stdout=$r.stdout;$result.stderr=$r.stderr};$result.was_alive=$alive;$result.status='PASS';$result.retryable=$false}
