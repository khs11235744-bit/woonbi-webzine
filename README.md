...
- `web/newsroom-v04.css`: 편집형 화면과 모바일 보정.
- `web/js/demo-store.js`: 로컬 임시·IndexedDB 저장.
- `web/js/firebase-store.js`, `firebase/`: 아직 실계정 검증을 마치지 않은 서버 연결 초안.
...
## 검증 범위
단위 검사는 저장·역할·본문 보호와 새 사진/편집 규칙을 다룹니다. 개인 검토본의 MemoryStore 브라우저 검사는 별도로 수행했습니다. Firebase 규칙 엔진·실제 Google 계정·학교 기기 간 동시 작성은 검증 대기입니다. 전체 WCAG 인증이나 인쇄소용 완전 자동조판을 주장하지 않습니다.

## 안전한 작업
전용 브랜치에서 작업하세요. 기존 원격 제어 저장소의 main으로 합치거나 기존 프로젝트의 배포 설정을 고치지 않습니다. 정식 독립 비공개 저장소로 옮길 때 이 코드 디렉터리만 사용합니다. 웹진은 아직 공개 배포하지 않았습니다.
