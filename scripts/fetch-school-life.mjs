...
function checkedAt(){
  return new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date()).replace(/\. /g,'.').replace(/\.$/,'');
}
...
async function neisMeal(apiKey,ymd){
  ...
  return {
    ...
    body:cap(stripAllergen(String(lunch.DDISH_NM||'').replace(/<br\s*\/?>/gi,' · ')),260),
    source:'나이스 급식식단정보',
    url:MEAL_PAGE
  };
}
async function officialMeal(homeHtml){
  ...
  return {
    title:'오늘의 급식',
    status:cleanHtml(dt)||'중식',
    body:cap(stripAllergen(cleanHtml(dd)),260),
    source:'포항고 공식 홈페이지 · 오늘의 식단',
    url:MEAL_PAGE
  };
}
async function neisSchedule(apiKey,ymd){
  ...
  return scheduleCard(items,'나이스 학사일정');
}
async function officialSchedule(ymd){
  ...
  return scheduleCard(items,'포항고 공식 홈페이지 · 학교일정');
}
function scheduleCard(items,source){
  const seen=new Set();
  items=items.filter(x=>{const k=x.date+'|'+String(x.title).replace(/\s+/g,'').toLowerCase();if(seen.has(k))return false;seen.add(k);return true;}).sort((a,b)=>a.date.localeCompare(b.date));
  const body=items.length?items.slice(0,6).map(x=>`${fmtDate(x.date)} ${x.title}`).join(' · '):'이번 주 등록된 주요 일정이 없습니다.';
  return {
    title:'이번 주 일정',
    status:items.length?`${items.length}건 확인`:'등록 일정 없음',
    body:cap(body,260),
    source,
    url:SCHEDULE_PAGE
  };
...
async function officialNotice(homeHtml){
  const section=homeHtml.match(/<div class="list_box[^"]*" id="notice1">([\s\S]*?)<\/div>\s*<div class="list_box[^"]*" id="notice2"/i)?.[1]
    || homeHtml.match(/id="notice1"([\s\S]*?)id="notice2"/i)?.[1]
    || homeHtml;
  const rows=[];
  const top=section.match(/<a\s+href="([^"]*selectNttInfo\.do\?[^"]+)"[^>]*class="[^"]*topList[^"]*"[^>]*>([\s\S]*?)<\/a>/i);
  if(top){
    const inner=top[2];
    const title=cleanHtml(inner.match(/<dt[^>]*>([\s\S]*?)<\/dt>/i)?.[1]||'');
  ...
  if(!rows.length)throw new Error('공식 공지사항 파싱 실패');
  const first=rows[0];
  return {
    title:'주요 공지',
    status:first.date||'최근 공지',
    body:cap(first.title,220),
    source:'포항고 공식 홈페이지 · 공지사항',
    url:first.url||NOTICE_LIST
  };
}
async function safe(name,fn,fallback){
  try{return await fn();}
    ...
    return {...fallback,status:'확인 실패',body:'공식 정보 연결을 잠시 확인하지 못했습니다.',error:true};
  }
}
...
const notice=await safe('notice',
  ...
  {title:'주요 공지',source:'포항고 공식 홈페이지 · 공지사항',url:NOTICE_LIST}
);
...
console.log(`meal=${meal.status}; schedule=${schedule.status}; notice=${notice.status}`);
