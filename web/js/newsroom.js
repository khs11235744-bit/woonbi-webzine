...
W.createNewsroom=function(api){
 ...
 const data=()=>W.newsroomData||{},archive=()=>W.archiveData||{};
 ...
 const label=a=>a.contentOrigin==='restored'?'복원 초안':a.contentOrigin==='new-editorial-draft'?'취재 초안':a.publishedAt?'발행 기사':'편집 초안';
 ...
 function headlink(a){return h('a',{href:'#read/'+encodeURIComponent(a.id),onclick:e=>{e.preventDefault();read(a.id).catch(toastError);}},a.title);}
 ...
 async function articles(){return storeFn().mode==='demo'?storeFn().listArticles():storeFn().listPublic();}
 ...
 async function articlePhoto(a,priority=false){
  const p=a.photos?.[0];if(p){const img=await imageNode(p);if(img.tagName==='IMG'){img.loading=priority?'eager':'lazy';if(priority)img.setAttribute('fetchpriority','high');img.decoding='async';}
   return h('figure',{class:'n-photo'},img,h('figcaption',{},E.sample(p)?h('span',{class:'sample-badge'},'교체용 예시 · 실제 현장 아님'):p.caption||'기사 사진'));}
  return h('div',{class:'v4-empty-image'},h('span',{},smallLabel(a)),h('small',{},'사진 준비 중'));
 }
 async function card(a,withPhoto=true){const art=h('article',{class:'n-card','data-read-card':a.id});
  if(withPhoto)art.append(h('a',{class:'n-card-image',href:'#read/'+a.id,'aria-label':a.title+' 기사 읽기',onclick:e=>{e.preventDefault();read(a.id).catch(toastError);}},await articlePhoto(a)));
 ...
 async function home(){
  ...
  if(storeFn().mode==='demo')main.append(h('div',{class:'n-edition-note'},h('b',{},'2026 편집 미리보기'),h('span',{},'미발행 원고 · 예시 사진은 교체 후 사용합니다.')));
  if(category!=='all'||q){
  ...
  const ids=data().leadIds||[],lead=rows.find(a=>a.id===ids[0])||rows[0],rest=rows.filter(a=>a.id!==lead.id);
  const sides=[...ids.slice(1).map(id=>rest.find(a=>a.id===id)).filter(Boolean),...rest].filter((a,i,ar)=>ar.findIndex(b=>b.id===a.id)===i).slice(0,2);
  ...
  leadArt.append(h('a',{class:'n-lead-image',href:'#read/'+lead.id,onclick:e=>{e.preventDefault();read(lead.id);}},await articlePhoto(lead,true)));
  ...
  if(token!==generation)return;main.append(h('section',{class:'n-front'},leadArt,side));
  ...
  main.append(sectionHead('더 읽을 기사',other.length+'편'),h('section',{class:'n-more-list'},other.slice(0,8).map(a=>h('article',{},h('span',{class:'n-section-label'},smallLabel(a)),h('h3',{},headlink(a)),meta(a)))));
 ...
 async function read(id,reference=false){
  ...
  const rendered=await renderArticle({...a,photos:a.photos||[],category:a.category||'예시 원고'}),copy=rendered.querySelector('.read-copy');copy.classList.add('n-story-copy');article.append(copy);
 ...
 async function examples(){cleanPage();main.append(h('header',{class:'n-page-heading'},h('span',{class:'n-section-label'},'REFERENCE DESK'),h('h1',{},'예시 원고 서가'),h('p',{},'백업에 남긴 원고 12편. 과거 사례와 올해 원고를 구분해 보관합니다.')));const list=h('div',{class:'n-example-list'});for(const a of data().examples||[])list.append(h('article',{},h('div',{},h('span',{class:'n-section-label'},a.archiveLabel||'참고 원고'),h('h2',{},h('a',{href:'#read/'+a.id,onclick:e=>{e.preventDefault();read(a.id,true);}},a.headline||a.title)),h('p',{},C.paragraphs(a.body)[0]?.slice(0,160)||'')),button('원고 읽기 →',()=>read(a.id,true),'text')));if(!list.children.length)list.append(h('p',{class:'empty'},'승인된 참고 원고를 연결하면 표시됩니다.'));main.append(list);}
