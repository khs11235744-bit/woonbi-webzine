...
const DEMO={
  mode:'demo',updatedAt:'',
  meal:{title:'오늘의 급식',status:'연결 준비 중',body:'NEIS 급식 API 연결 후 오늘 중식이 표시됩니다.',source:'나이스 급식식단정보'},
  schedule:{title:'이번 주 일정',status:'연결 준비 중',body:'포항고 학사일정과 주요 행사를 이곳에 표시합니다.',source:'포항고 공식 일정'},
  notice:{title:'주요 공지',status:'편집자 선택',body:'학교 공지 중 학생에게 필요한 소식만 짧게 연결합니다.',source:'포항고 공지사항'}
};
...
async function build(){
  ...
  head.append(left,el('p','',d.mode==='live'?(d.updatedAt?`확인 ${d.updatedAt}`:'공식 정보 연결'):'연결 준비 중 · 기사와 별도'));
  wrap.append(head);const grid=el('div','school-life-grid');
  for(const key of ['meal','schedule','notice']){const x=d[key]||{},a=el('article','school-life-item');a.append(el('span','school-life-status',text(x.status)),el('h3','',text(x.title)),el('p','',text(x.body)),el('small','',text(x.source)));grid.append(a);}wrap.append(grid);
  if(d.error)wrap.append(el('p','school-life-error',d.error));return wrap;
}
let seq=0;
